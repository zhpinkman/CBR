...
from transformers import (TrainerCallback, PretrainedConfig,
                          Trainer, TrainingArguments, BertModel, BertPreTrainedModel, BertTokenizer)
...
def save_results(config, label_encoder, predictions, test_df):
    ...
    outputs_dict = {}
    outputs_dict['note'] = 'bert_model_with_attention_check_cbr_different_features_for_retrieval'
    ...
    with wandb.init(config=config):
        ...
        def process(batch):
            ...
            inputs_cbr = tokenizer(
                batch["augmented_cases"], truncation=True, padding='max_length', max_length=512
            )
            return {
                'input_ids': inputs['input_ids'],
                'attention_mask': inputs['attention_mask'],
        ...
        training_args = TrainingArguments(
            do_eval=True,
            do_train=True,
            output_dir=f"./cbr_bert_logical_fallacy_classification_{config.data_dir.replace('/', '_')}",

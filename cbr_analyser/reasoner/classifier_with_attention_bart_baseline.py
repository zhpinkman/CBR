...
from transformers import (TrainerCallback, BartForSequenceClassification,
                          Trainer, TrainingArguments, BartTokenizer)


checkpoint_for_adapter = "ynie/bart-large-snli_mnli_fever_anli_R1_R2_R3-nli"
...
def save_results(config, label_encoder, predictions, predictions_climate, test_df):
    ...
    outputs_dict = {}
    outputs_dict['note'] = 'bart_model_with_attention_check_cbr_different_features_for_retrieval_baseline'
...
def do_train_process(config=None):
    with wandb.init(config=config):
        ...
        config = wandb.config
        tokenizer = BartTokenizer.from_pretrained(checkpoint_for_adapter)
        ...
        training_args = TrainingArguments(
            do_eval=True,
            do_train=True,
            output_dir=f"./cbr_bart_logical_fallacy_classification_{config.data_dir.replace('/', '_')}",
